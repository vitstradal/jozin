...
public class TabbedPrintWriter {
	...
	public void dumpRegion(String line) {
	    ...
	    Enumeration eenum = childBPs.elements();
	    int cur = startPos;
	    BreakPoint child = (BreakPoint) eenum.nextElement();
	    if (child.startPos >= 0) {
		...
		child = (BreakPoint) eenum.nextElement();
...
//  		       +space+","+lastSpace+";"+minPenalty);
	    ...
	    for (lastChild = (BreakPoint) eenum.nextElement();
		 eenum.hasMoreElements(); lastChild = nextChild) {
		nextChild = (BreakPoint) eenum.nextElement();
