...
public class Main extends Options {
	...
	if (failedClasses.size() > 0) {
	    ...
	    Enumeration eenum = failedClasses.elements();
	    while (eenum.hasMoreElements()) {
		GlobalOptions.err.println("\t" + eenum.nextElement());
	    }
	...
	if (destDir == null)
		...
		if ((params[i].endsWith(".jar") || params[i].endsWith(".zip"))
		    ...
		    ClassInfo.setClassPath(params[i]
					   + Decompiler.altPathSeparatorChar
					   + classPath);
		    Enumeration eenum = new ZipFile(params[i]).entries();
		    while (eenum.hasMoreElements()) {
			String entry
			    = ((ZipEntry) eenum.nextElement()).getName();
