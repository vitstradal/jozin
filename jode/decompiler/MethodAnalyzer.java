...
public class MethodAnalyzer implements Scope, ClassDeclarer {
	...
	if (innerAnalyzers != null) {
	    for (Enumeration eenum = innerAnalyzers.elements();
		 eenum.hasMoreElements(); ) {
		ClassAnalyzer classAna = (ClassAnalyzer) eenum.nextElement();
        ...
        for (Enumeration eenum = allLocals.elements();
	     eenum.hasMoreElements(); ) {
            LocalInfo li = (LocalInfo)eenum.nextElement();
            if (!li.isShadow())
    ...
    public LocalInfo findLocal(String name) {
        Enumeration eenum = allLocals.elements();
	...
	if (innerAnalyzers != null) {
	    Enumeration eenum = innerAnalyzers.elements();
	    while (eenum.hasMoreElements()) {
		ClassAnalyzer classAna = (ClassAnalyzer) eenum.nextElement();
		if (classAna.getParent() == this
