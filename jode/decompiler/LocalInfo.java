...
// 				  +shadow.name+"("+shadow.type+")");
	...
	java.util.Enumeration eenum = operators.elements();
	while (eenum.hasMoreElements()) {
	    LocalVarOperator lvo = 
		(LocalVarOperator) eenum.nextElement();
	    if (needTypeUpdate) {
	...
	eenum = hints.elements();
	while (eenum.hasMoreElements()) {
	    Object hint = eenum.nextElement();
	...
	if (name == null) {
	    Enumeration eenum = hints.elements();
	    while (eenum.hasMoreElements()) {
		Hint hint = (Hint) eenum.nextElement();
	...
	if (newType == Type.tError
        ...
        if (!li.type.equals(newType)) {
            li.type = newType;
            java.util.Enumeration eenum = li.operators.elements();
            while (eenum.hasMoreElements()) {
                LocalVarOperator lvo = (LocalVarOperator) eenum.nextElement();
                if ((GlobalOptions.debuggingFlags & GlobalOptions.DEBUG_TYPES) != 0)
                    GlobalOptions.err.println("updating "+lvo);
                lvo.updateType();
            }
	...
	LocalInfo li = getLocalInfo();
	Enumeration eenum = li.operators.elements();
	int writes = 0;
	while (eenum.hasMoreElements()) {
	    if (((LocalVarOperator) eenum.nextElement()).isWrite())
		writes++;
	}
